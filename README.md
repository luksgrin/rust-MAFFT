...
Remaining divergence (**729** vs C's 721, 1.1%): every individually testable component has been FFI-validated byte-identical to C — per-branch weights (`weightFromABranch`, exact match to 2.22e-16 across all 70 branches of 36-seq topology), per-group normalization (`fastconjuction_noname`), profile construction (`cpmx_calc_new` + gap counts), intergroup scoring (`intergroup_score`), and profile alignment (`MSalignmm`, byte-identical at every branch of iter 0 on both clean and evolving state). The residual gap comes from a code-path difference: C's `Falign` runs segmented DP on the **full** (unstripped) sequences with FFT anchors determining segment boundaries, while we run `profile_align` on per-group-stripped profiles (or anchored DP on full profiles when the FFT finds anchors). The stripped vs full DP produces slightly different alignments at a few late branches, which cascade through the 16 iterations × 69 branches. Fixing this requires porting C's full `Falign` segment logic; see `TODO.md`. Progressive alignment (single pass) remains byte-identical to C.
