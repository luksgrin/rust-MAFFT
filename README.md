...
Remaining divergence (**729** vs C's 721, 1.1%): every individually testable component has been FFI-validated byte-identical to C — per-branch weights (`weightFromABranch`, exact match to 2.22e-16 across all 70 branches of 36-seq topology), per-group normalization (`fastconjuction_noname`), profile construction (`cpmx_calc_new` + gap counts), intergroup scoring (`intergroup_score`), and profile alignment (`MSalignmm`, byte-identical at every branch of iter 0 on both clean and evolving state). This session ported C's dvtditr-specific `Falign` flow exactly (`kobetsubunkatsu=1` skips FFT; uses `alignableReagion` at lag=0 to find segment centers; per-segment `commongappick` strips per-group all-gap columns; per-segment DP). See `TODO.md` for the remaining investigation: 15/207 multi-segment branches still need boundary gap state (`sgap/egap` → `new_OpeningGapCount`) to fully match C, or `alignableReagion` segment centers differ by FP and need further verification. Progressive alignment (single pass) remains byte-identical to C.
