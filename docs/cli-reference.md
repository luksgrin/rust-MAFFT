...
Options:
  ...
  -q, --quiet                        Quiet mode: suppress progress messages
      ...
      --nwildcard                    Treat N (DNA/RNA ambiguous) as a wildcard that matches anything positively (mirrors C MAFFT's `--nwildcard`, internal flag `-:`). Currently accepted but the N-row scoring tweak is not yet wired; affects only DNA workflows. See `TODO.md`
      --nzero                        Treat N (DNA/RNA ambiguous) as scoring 0 against everything (mirrors C MAFFT's `--nzero`). Currently accepted but the N-row scoring tweak is not yet wired. See `TODO.md`
      ...
      --youngestlinkage              Use the "youngest" linkage scheme (matches C `--youngestlinkage`). C's `youngestlinkage` is a separate algorithm, not just a `sueff` value, and is NOT yet ported. Flag accepted as a no-op for compatibility. See `TODO.md`
      --bestfirst                    Use C MAFFT's `BESTFIRST` parallelisation strategy for iterative refinement — score every branch first, then refine in score-order. Default is `BAATARI2` (simple hill climbing). Currently accepted as a no-op; the BESTFIRST refinement loop architecture is a separate port. See `TODO.md`
      ...
      --skipiterate <F>              Skip refinement of branches whose distance-from-tip exceeds F (matches C `--skipiterate F` → `dvtditr -E $fixthreshold`). Currently accepted as a no-op; the branch-skip gate is not yet wired into the refinement loop. See `TODO.md`
      --oneiteration                 Run only one iteration of the disttbfast distance refinement (matches C `--oneiteration` → `disttbfast -r`). Currently accepted as a no-op; our distance recomputation already runs once per retree pass — closer investigation needed before forcing a single iteration here. See `TODO.md`
      --adjustdirection              Auto-detect input DNA strand orientation and reverse-complement sequences on the wrong strand before alignment (matches C `--adjustdirection`). Currently accepted at the CLI but the k-mer-based detection algorithm (port of `mafft-upstream/core/makedirectionlist.c`) is not yet implemented. Affects DNA workflows only; protein inputs silently bypass it. See `TODO.md`
      --adjustdirectionaccurately    Slower, more accurate variant of `--adjustdirection` (matches C `--adjustdirectionaccurately`, internally `adjustdirection=2`). Same stub status — not yet implemented
